...
mod tests {
    ...
    #[test]
    fn sync_committee_contribution_json_uses_wire_encoding() {
        ...
        assert_eq!(contribution.slot, 9);
        assert_eq!(contribution.subcommittee_index, 3);
        assert_eq!(contribution.beacon_block_root, [0x66; 32]);
        assert_eq!(contribution.signature, [0x77; 96]);
        assert_eq!(
            serde_json::to_value(&contribution).expect("serialize"),
            wire
        );
    }

    #[test]
    fn sync_committee_contribution_json_rejects_bad_bits_length() {
        ...
        serde_json::from_value::<SyncCommitteeContribution>(wire)
            .expect_err("wrong bit vector length must fail");
    }
}

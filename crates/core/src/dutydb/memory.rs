...
) -> std::result::Result<UnsignedDataSet, ParSigExCodecError> {
    if set.set.is_empty() {
...
fn unsigned_duty_data_from_proto(
    duty_type: &DutyType,
    data: &[u8],
) -> std::result::Result<UnsignedDutyData, ParSigExCodecError> {
    match duty_type {
...
fn decode_attestation_data(
    data: &[u8],
) -> std::result::Result<AttestationData, ParSigExCodecError> {
...
fn decode_attestation_data_ssz(
    data: &[u8],
) -> std::result::Result<AttestationData, ParSigExCodecError> {
...
fn decode_attester_duty_ssz(data: &[u8]) -> std::result::Result<AttesterDuty, ParSigExCodecError> {

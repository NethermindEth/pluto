...
pub async fn run(conf: Config, ct: CancellationToken) -> Result<(), DkgError> {
    ...
    // Charon, which bounds the DKG by `conf.Timeout`. Every phase (peer sync,
    // FROST rounds, partial-signature exchanges) observes this token. A single
    // overall deadline is used rather than per-phase deadlines because the sync
    // service spans all phases and must not be cancelled mid-ceremony.

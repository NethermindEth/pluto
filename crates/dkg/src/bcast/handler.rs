...
async fn handle_inbound_sig_request(
    ...
    let request = timeout(
        RECEIVE_TIMEOUT,
        pluto_p2p::proto::read_protobuf_with_max_size::<BCastSigRequest, _>(
            &mut stream,
    ...
    timeout(
        RECEIVE_TIMEOUT,
        pluto_p2p::proto::write_protobuf(&mut stream, &response),
    )
    .await
...
async fn handle_inbound_msg(
    ...
    let message = timeout(
        RECEIVE_TIMEOUT,
        pluto_p2p::proto::read_protobuf_with_max_size::<BCastMessage, _>(
            &mut stream,
            protocol::MAX_MESSAGE_SIZE,
        ),
    )

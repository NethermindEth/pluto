...
//! - `Component` is the caller-facing handle. It owns the message registry and
//!   submits outbound broadcasts to the swarm-owned `Behaviour`.
...
//!
//! API choice:
//! - `Component::broadcast` only enqueues the broadcast. Use it when the caller
//!   observes `Event` separately or intentionally does not need to wait for
//!   terminal status.

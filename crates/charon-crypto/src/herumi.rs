...
impl Tbls for Herumi {
    ...
    fn threshold_split_insecure(
        ...
        // blsful uses u8 for share IDs, limiting us to 255 shares max
        if total > 255 {
        ...
        for (i, share) in shares.iter().enumerate() {
            ...
            // Copy bytes 1..33 (skip the ID byte)
            share_secret_key.copy_from_slice(&share_vec[1..33]);
    ...
    fn recover_secret(&self, shares: HashMap<u64, PrivateKey>) -> Result<PrivateKey, Error> {
        ...
        for (id, share) in shares.iter() {
            // Validate share ID fits in u8 (blsful limitation)
            if *id >= 255 {
                return Err(Error::InvalidThreshold);
            }
            ...
            let mut full_share = Vec::with_capacity(33);
            full_share.push((*id + 1) as u8); // Convert 0-indexed to 1-indexed
            full_share.extend_from_slice(share);

            match SecretKeyShare::<Bls12381G2Impl>::try_from(full_share.as_slice()) {
    ...
    fn aggregate(&self, signatures: Vec<Signature>) -> Result<Signature, Error> {
        ...
        if signatures_vec.len() == 1 {
            let signature_vec = Vec::from(signatures_vec[0]);
            let mut signature = [0u8; 97];
            signature.copy_from_slice(&signature_vec);
            return Ok(signature);
        }

        let signature = blsful::AggregateSignature::from_signatures(&signatures_vec)
    ...
    fn threshold_aggregate(
        ...
        for (id, signature) in partial_signatures_by_idx.iter() {
            ...
            let mut share_sig = Vec::with_capacity(98);
            share_sig.push(signature[0]); // Keep the header byte
            share_sig.push((*id + 1) as u8); // Insert share ID (1-indexed)
            share_sig.extend_from_slice(&signature[1..]); // Add the 96 signature bytes

            match SignatureShare::<Bls12381G2Impl>::try_from(share_sig.as_slice()) {
    ...
    fn verify_aggregate(
        ...
        let signature = MultiSignature::try_from(signature.as_slice())
            .map_err(|_| Error::FailedToDeserializeSignatureKey)?;
...
mod tests {
    ...
    // =========================================================================
    // Original 1-to-1 Tests (from Go implementation)
    // These tests match the original charon-go test suite for compatibility
    // =========================================================================
    ...
    fn test_original_recover_secret() {
        ...
        let subset: HashMap<u64, PrivateKey> =
    ...
    // =========================================================================
    // Secret to Public Key Conversion Tests
    // =========================================================================
    ...
    fn test_secret_to_public_key_invalid_secret_key() {
        ...
        assert_eq!(result.unwrap_err(), Error::FailedToDeserializeSecretKey);
    }
    ...
    fn test_verify_fails_with_wrong_public_key() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), Error::FailedToVerifySignature);
    }
    ...
    #[test]
    fn test_threshold_split_max_shares_limit() {
        let herumi = setup();
        let sk = herumi
            .generate_secret_key(&mut elliptic_curve::rand_core::OsRng)
            .unwrap();

        // 255 shares should work (blsful uses u8 for share IDs)
        let result = herumi.threshold_split(&sk, 255, 128);
        assert!(result.is_ok());

        // 256 shares should fail
        let result = herumi.threshold_split(&sk, 256, 128);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), Error::InvalidThreshold);

        // Much larger should also fail
        let result = herumi.threshold_split(&sk, 1000, 500);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), Error::InvalidThreshold);
    }

    #[test]
    fn test_recover_secret_with_invalid_share_id() {
        let herumi = setup();
        let mut shares = HashMap::new();
        
        // Create a share with ID >= 255
        shares.insert(255, [1u8; 32]);
    ...
    fn test_recover_secret_with_exact_threshold() {
        ...
        let subset: HashMap<u64, PrivateKey> = shares
            .iter()
    ...
    fn test_recover_secret_different_share_combinations() {
        ...
        let subset1: HashMap<u64, PrivateKey> =
        ...
        let subset2: HashMap<u64, PrivateKey> =
        ...
        let subset3: HashMap<u64, PrivateKey> =
    ...
    fn test_aggregate_with_invalid_signature() {
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), Error::FailedToDeserializeSignatureKey);
    }

    // =========================================================================
    // Verify Aggregate Tests
    // =========================================================================
    ...
    fn test_threshold_aggregate_different_share_combinations() {
        ...
        for combo in 0..3 {
            ...
            let subset: HashMap<u64, PrivateKey> = shares_vec[start..end]
                .iter()
                .map(|(k, v)| (**k, **v))
                .collect();

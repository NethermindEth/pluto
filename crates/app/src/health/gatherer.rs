...
//! the registered base name (and Charon).
...
///
/// Best-effort: comment lines other than `# TYPE` are skipped, and sample lines
/// that fail to parse are dropped. Each `# TYPE` line starts a new family;
/// subsequent sample lines are grouped under it.
fn parse_exposition(text: &str) -> Vec<MetricFamily> {
    ...
    for line in text.lines() {
        ...
        if let Some(rest) = line.strip_prefix("# TYPE ") {
            ...
            match parse_sample(line, family.metric_type) {
                Some(metric) => family.metrics.push(metric),
                // Input is trusted (we encode it ourselves), so a failure here
                // means a parser gap. Surface it at debug rather than dropping
                // silently (which could mask a queried metric). Not warn/error:
                // those would feed back into `app_log_*_total` and the checker.
                None => {
...
fn parse_sample(line: &str, metric_type: MetricType) -> Option<Metric> {
    let (labels, value_str) = match line.split_once('{') {
        Some((_name, rest)) => {
    ...
    Some(make_metric(labels, value, metric_type))
}
...
mod tests {
    use super::*;
    use vise::{Counter, Gauge, LabeledFamily, Metrics, Registry};
    ...
    #[test]
    fn gather_emits_charon_names_and_strips_total() {

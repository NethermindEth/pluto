...
/// Mounts POST `/eth/v1/beacon/states/{state_id}/validators` (the endpoint the
/// `ValidatorCache` queries) returning ONLY the datums whose pubkey appears in
/// the request-body `ids` — so an unseeded (empty-pubkey) cache resolves zero
/// validators.

...
//!
//! Scope is readiness only. It does NOT assert duty completion: this node is
//! operator 0 of a 2-of-3 cluster, so it cannot reach QBFT quorum alone, and it
//! is cancelled as soon as the validator API binds — before the validator mock
//! drives its first (post-startup-delay) duty.
//! (`scripts/dkg-runner/ci/verify-run-smoke.sh` likewise checks readiness only,
//! not real beacon duties.) The cross-node duty-submission path is covered by
//! `multinode_parsig_exchange_reaches_submission` in `tests/wiring.rs`, which
//! routes partial signatures between N wired nodes and asserts the aggregated
//! attestation reaches the beacon mock's submit endpoint.

...
use crate::{
    tbls::Tbls,
    types::{
        BlsError, Error, Index, PrivateKey, PublicKey, SCALAR_LENGTH, SIGNATURE_LENGTH, Signature,
    },
};
...
impl Tbls for BlstImpl {
    ...
    fn threshold_split_insecure(
        ...
        // Evaluate polynomial at points 1..total to create shares. The fr-domain
        // copy of the secret coefficients is wiped on drop.
        let poly_fr = SecretFrVec::from_secrets(&poly);

        let mut shares = HashMap::new();
        for i in 1..=total {
            let mut share_fr = evaluate_polynomial_fr(&poly_fr.0, i)?;
            let share = secret_from_fr(&share_fr);
            wipe_fr(std::slice::from_mut(&mut share_fr));
    ...
    fn recover_secret_from_every_threshold_subset() {
        ...
        for (total, threshold) in [(4u64, 2u64), (5, 3), (6, 4), (7, 4)] {
            ...
            for subset in combinations(&indices, usize::try_from(threshold).unwrap()) {
                ...
                let recovered = blst.recover_secret(&picked).unwrap();
                assert_eq!(
                    secret, recovered,
    ...
    fn threshold_aggregate_from_every_threshold_subset() {
        ...
        for (total, threshold) in [(4u64, 2u64), (5, 3), (6, 4), (7, 4)] {
            ...
            let partials: HashMap<Index, Signature> = shares
                .iter()
                .map(|(idx, key)| (*idx, blst.sign(key, data).unwrap()))
                .collect();
            let indices: Vec<Index> = {
            ...
            for subset in combinations(&indices, usize::try_from(threshold).unwrap()) {
                ...
                let aggregated = blst.threshold_aggregate(&picked).unwrap();
                assert_eq!(

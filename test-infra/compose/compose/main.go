...
func newNewCmd() *cobra.Command {
	...
	nodeImpls := cmd.Flags().StringSlice("node-impls", nil, "Node implementations (charon or pluto) assigned round-robin to nodes; e.g. charon,pluto for alternating. Empty defaults to all charon.")
	keygenImpl := cmd.Flags().String("keygen-impl", string(conf.KeyGenImpl), "Implementation running single-container keygen steps: charon or pluto. Empty defaults to node0's implementation.")
	plutoImageTag := cmd.Flags().String("pluto-image-tag", conf.PlutoImageTag, "Pluto docker image tag: pluto:{tag}. Tag 'local' is built from source, which requires the PLUTO_REPO env var.")
	...
	cmd.RunE = func(cmd *cobra.Command, _ []string) error {
		...
		conf.KeyGenImpl = compose.NodeImpl(*keygenImpl)
		conf.PlutoImageTag = *plutoImageTag

		if len(*nodeImpls) > 0 {
			var impls []compose.NodeImpl
			for _, impl := range *nodeImpls {
				impls = append(impls, compose.NodeImpl(impl))
			}

			conf.NodeImpls = impls
		}
